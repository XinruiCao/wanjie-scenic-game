...
try {
 ...
 engine.enter('EP12');assert.equal(s.metaFlags.darkSkinUnlocked,undefined,'visiting decision must not unlock')
 engine.applyChoice('EP12_A');assert.equal(s.metaFlags.darkSkinUnlocked,true);assert.equal(s.metaFlags.scenicSkin,'dark')
 ...
 const old=initialState();old.archives.push({playthrough:1,run:{...freshRun(),choiceHistory:[{node:'EP12',choice:'EP12_B',at:1}]}});assert.equal(syncSkinProgress(old),true)
 const jump=initialState();jump.visitedNodes=['EP12','EP13'];assert.equal(syncSkinProgress(jump),false,'debug jumps are not completion')
 const invalid=initialState();invalid.choiceHistory.push({node:'EP12',choice:'invalid',at:1});assert.equal(syncSkinProgress(invalid),false)
