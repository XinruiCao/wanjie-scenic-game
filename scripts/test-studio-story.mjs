...
p=copy();p.nodes.C_M01.choices[0].next='';assert.ok(validate(p).errors.length)
p=copy();p.nodes.V_M01.next='V_M01';assert.ok(validate(p).errors.some(s=>s.includes('无法到达')))
p=copy();p.nodes.V_TRUE.next='V_TRUE';assert.ok(validate(p).errors.some(s=>s.includes('无法到达结局')))
p=copy();delete p.nodes.V_M01;assert.ok(validate(p).errors.some(s=>s.includes('入口')))
p=copy();p.nodes.C_M01.choices.push({...p.nodes.C_M01.choices[0]});assert.ok(validate(p).errors.some(s=>s.includes('ID 重复')))
...
p=copy();p.nodes.V_M01.notes='PRIVATE_EDITOR_NOTE';p.nodes.C_M01.choices[0].notes='PRIVATE_CHOICE_NOTE';p.layout.V_M01={x:123,y:456};p.trash.push({notes:'PRIVATE_TRASH'});const runtime=compile(p);assert.ok(!JSON.stringify(runtime).includes('PRIVATE_'));assert.deepEqual(runtime,JSON.parse(await readFile(new URL('../src/data/story.runtime.json',import.meta.url),'utf8')));console.log('PASS: runtime matches existing game; notes, layout and trash excluded; conditions and effects preserved')
